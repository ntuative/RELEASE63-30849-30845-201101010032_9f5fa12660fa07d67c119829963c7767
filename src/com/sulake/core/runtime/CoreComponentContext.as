package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1189:String = "asset-libraries";
      
      private static const const_1194:String = "library";
      
      private static const const_1188:String = "service-libraries";
      
      private static const const_1196:String = "library";
      
      private static const const_1195:String = "component-libraries";
      
      private static const const_1190:String = "library";
      
      private static const const_1193:String = "error_data";
      
      private static const const_1191:String = "error_cat";
      
      private static const const_1192:String = "error_desc";
       
      
      private var var_322:LibraryLoaderQueue;
      
      private var var_697:IEventDispatcher;
      
      private var var_474:uint;
      
      private var var_473:Function;
      
      private var var_244:Profiler;
      
      private var var_475:Array;
      
      private var var_578:Array;
      
      private var var_1707:uint;
      
      private var var_1442:uint = 0;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1104:
               debug("Core; using simple frame update handler");
               this.var_473 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_646:
               debug("Core; using complex frame update handler");
               this.var_473 = this.complexFrameUpdateHandler;
               break;
            case Core.const_1491:
               debug("Core; using profiler frame update handler");
               this.var_473 = this.profilerFrameUpdateHandler;
               this.var_244 = new Profiler(this);
               attachComponent(this.var_244,[new IIDProfiler()]);
               var_415.addChild(new ProfilerViewer(this.var_244));
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_473 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_322.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_474 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1193,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1191,String(param3));
         ErrorReportStorage.setParameter(const_1192,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_697 = loadingEventDelegate;
         if(this.var_322 == null)
         {
            this.var_322 = new LibraryLoaderQueue(_debug);
         }
         try
         {
         ...
         try
         {
            node = config.child(const_1188)[0];
            if(node != null)
            {
               list = node.child(const_1196);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1195)[0];
            if(node != null)
            {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1463);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_697)
         {
            this.var_697.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_697)
         {
            ...
            this.var_697.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_697)
                  {
                     this.var_697.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_474 - this.var_322.length,this.var_474,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_473(_loc2_,_loc2_ - this.var_1707);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(criticality < CoreComponentContext.const_141)
         {
            ...
            {
               if(this.var_578[criticality] < criticality)
               {
                  ++this.var_578[criticality];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(criticality < CoreComponentContext.const_141)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_244.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_244)
            {
               detachComponent(this.var_244);
               this.var_244.dispose();
               this.var_244 = null;
            }
            switch(this.var_1442 & 0)
            {
               case Core.const_1104:
                  this.var_473 = this.simpleFrameUpdateHandler;
                  break;
