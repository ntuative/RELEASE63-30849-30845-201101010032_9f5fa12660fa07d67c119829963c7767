package com.sulake.core.window.tools
{
   ...
   public class ProfilerOutput implements IDevTool
   {
      ...
      private static var var_1219:Class = ProfilerOutput_profiler_dialog_xml;
      
      private static var var_1220:Class = ProfilerOutput_profiler_task_xml;
       
      
      private var _disposed:Boolean = false;
      
      private var var_137:IProfiler;
      ...
      {
         super();
         this.var_22 = param1 as ICore;
         this.var_2688 = new Array();
         this.var_137 = this.profiler;
         this._windowManager = param2;
         this.var_826 = param3;
         var _loc4_:ByteArray = new var_1219() as ByteArray;
         ...
         ITextWindow(this._window.findChildByName("header")).text = padAlign("task",28) + "|" + padAlign("#rounds",8) + "|" + padAlign("latest ms",8) + "|" + padAlign("total ms",8) + "|";
         ITextWindow(this._window.findChildByName("footer")).text = "<- Click to enable bitmap memory tracking";
         ITextWindow(this._window.findChildByName("footer")).textColor = 4284900966;
      }
      ...
      public function set visible(param1:Boolean) : void
      {
         ...
         if(value != this.visible)
         {
            ...
            if(value)
            {
               ...
               this.var_22.setProfilerMode(true);
               this.var_22.queueInterface(new IIDProfiler(),function(param1:IID, param2:IUnknown):void
               {
            ...
            {
               this.var_22.setProfilerMode(false);
            }
         }
      }
      ...
      public function set profiler(param1:IProfiler) : void
      {
         if(this.var_137)
         {
            this.var_137.removeStopEventListener(this.refresh);
            this.var_137 = null;
         }
         if(!this.var_137 && param1)
         {
            this.var_137 = param1;
            this.var_137.addStopEventListener(this.refresh);
         }
      }
      ...
      private function profilerWindowEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param2.name == "footer_enable_toggle")
         {
            ...
            {
               this.var_1474 = true;
               ITextWindow(this._window.findChildByName("footer")).textColor = 4278190080;
            }
            else if(param1.type == WindowEvent.const_746)
            {
               this.var_1474 = false;
               ITextWindow(this._window.findChildByName("footer")).textColor = 4284900966;
            }
      ...
      {
         var _loc2_:Array = this.var_137.getProfilerAgentsInArray();
         ...
         if(this.var_1474)
         {
            ITextWindow(this._window.findChildByName("footer")).text = "Assets - Libraries: " + this.var_137.numAssetLibraryInstances + " " + "Bitmaps: " + this.var_137.numBitmapAssetInstances + " / " + this.var_137.numAllocatedBitmapDataBytes + " bytes \n" + "Windows - Allocated bitmap data: " + (this.var_826.allocatedByteCount + GraphicContext.allocatedByteCount) + " bytes \n" + "Tracked bitmap data: " + this.var_137.numTrackedBitmapDataInstances + " / " + this.var_137.numTrackedBitmapDataBytes + " bytes";
         }
      }
      ...
      private function recursiveRedraw(param1:ProfilerAgentTask, param2:IItemListWindow, param3:uint, param4:uint) : uint
      {
         ...
         _loc7_.text = padAlign(_loc6_,28," ",false) + "|" + padAlign(String(param1.rounds),8," ",true) + "|" + padAlign(String(param1.latest),8," ",true) + "|" + padAlign(String(param1.total),8," ",true) + "|\r";
      ...
      {
         var _loc2_:ByteArray = new var_1220() as ByteArray;
