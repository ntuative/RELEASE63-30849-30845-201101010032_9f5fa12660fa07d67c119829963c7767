package com.sulake.core.window.graphics
{
   ...
   public class DrawBufferAllocator implements IDisposable
   {
      ...
      public function get allocatedByteCount() : uint
      {
         ...
         var bytes:uint = 0;
         var index:uint = this.var_568.length;
         try
         {
            while(index > 0)
            {
               index--;
               bitmap = BitmapData(this.var_568[index]);
      ...
      public function dispose() : void
      {
         ...
         {
            this._disposed = true;
            _loc1_ = uint(this.var_568.length);
            while(_loc1_ > 0)
            {
               BitmapData(this.var_568.pop()).dispose();
               _loc1_--;
            }
         }
      }
