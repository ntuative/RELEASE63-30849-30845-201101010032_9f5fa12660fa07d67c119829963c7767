package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_335:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_335 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_597 && this.var_1383)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_82,this.var_907,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_33,parseInt(FigureData.const_787));
         ...
         if(this.var_335)
         {
            if(this.var_335.bitmap)
            {
               this.var_335.bitmap.dispose();
            }
            this.var_335.bitmap = new BitmapData(this.var_335.width,this.var_335.height,true,0);
            _loc7_ = (this.var_335.width - _loc1_.width) / 2;
            _loc8_ = (this.var_335.height - _loc1_.height) / 2;
            this.var_335.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_597 = this._controller.figureData.getFigureString();
               this.var_907 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1760,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_597)
               {
                  this._controller.loadAvatarInEditor(this.var_597,this.var_907,this._controller.clubMemberLevel);
               }
         }
      }
