package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_357.getValue(_loc4_) == null)
               {
                  this.var_357.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         if(param1 == null || this.var_425 == null)
         {
            return;
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_425.length > 0)
         {
            _loc1_ = this.var_425.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_357.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
            this.assetsReady(param1,AvatarScaleType.const_49,param2);
         }
         return _loc3_;
      }
      ...
      {
         var _loc4_:PetData = this.var_357.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_357.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_357.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.name_4.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1208.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_527.getValue(_loc6_) == null)
               {
                  ...
                  this.var_257.push(_loc8_);
                  this.var_527.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_425.indexOf(param3) == -1)
                  {
                     this.var_425.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_527.getKeys())
         {
            _loc5_ = this.var_527.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
      ...
      private function onContentLoadError(param1:Event) : void
      {
         ...
         var _loc3_:int = this.var_257.indexOf(_loc2_);
         if(_loc3_ != -1)
         {
