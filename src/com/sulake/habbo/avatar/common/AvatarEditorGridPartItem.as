package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_685:IFigurePartSet;
      ...
      private var var_383:IAvatarRenderManager;
      
      private var var_687:Boolean;
      
      private var var_1638:BitmapData;
      
      private const const_1854:int = 13421772;
      
      private const const_1853:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this._window = param1;
         this._colors = param4;
         this.var_1636 = param5;
         if(param3 == null)
         {
            this.var_631 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1637 = Math.max(this.var_1637,_loc6_.colorLayerIndex);
            }
         }
         this.var_383 = this.var_62.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_631 != null && !this.var_1636)
            {
               _loc3_ = this.var_631;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_685)
            {
               switch(this.var_685.clubLevel)
               {
                  case HabboClubLevelEnum.const_46:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_58:
                     _loc2_.style = HabboIconType.const_621;
                     break;
                  case HabboClubLevelEnum.const_53:
                     _loc2_.style = HabboIconType.const_692;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1854;
         }
         else
         {
            this._window.color = this.const_1853;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:AvatarFigureContainer = this.var_383.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_383.isFigureReady(_loc1_))
         {
            this.var_383.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_309[_loc6_] + "_" + FigureData.const_134;
               _loc4_ = this.var_383.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_565 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1638)
               {
                  ...
                  this.var_1638 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1636 && _loc2_.colorLayerIndex > 0)
               {
