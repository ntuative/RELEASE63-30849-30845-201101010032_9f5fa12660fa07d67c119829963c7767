package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_816:int = 3;
      
      private var var_364:AssetLoaderStruct;
      ...
      private const const_1838:int = 100;
      
      private const const_1314:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_364 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_815 = new Timer(this.const_1838,1);
         this.var_815.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_364)
         {
            this.var_364.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_364.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_364)
         {
            this.var_364.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_364.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_816);
            if(this.var_1250.indexOf("?") > 0)
            {
               _loc2_ = this.var_1250 + "&retry=" + this.var_816;
            }
            else
            {
               _loc2_ = this.var_1250 + "?retry=" + this.var_816;
            ...
            this.var_364 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1533 = true;
         for each(_loc2_ in this.var_989)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2086,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_988[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_988[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1533)
         {
            this.var_989.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_425[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_425[_loc3_] = _loc6_;
            }
            this.var_814[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_814)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_425[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_814[_loc3_];
         }
         while(_loc6_ < this.var_651.length)
         {
            _loc5_ = this.var_651[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_651.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_815.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_652.length > 0 && this.var_651.length < this.const_1314)
         {
            _loc1_ = this.var_652[0];
            _loc1_.startDownloading();
            this.var_651.push(this.var_652.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_652.indexOf(param1) == -1 && this.var_651.indexOf(param1) == -1)
         {
