package com.sulake.habbo.inventory.marketplace
{
   ...
   public class MarketplaceView implements IGetImageListener
   {
      ...
      private var var_62:MarketplaceModel;
      ...
      {
         var _loc4_:* = null;
         if(!param1 || !this._localization || !this._roomEngine || !this.var_62)
         {
            return;
         }
         ...
         this.checkPrice();
         this._localization.registerParameter("inventory.marketplace.make_offer.expiration_info","time",this.var_62.expirationHours.toString());
         this._localization.registerParameter("inventory.marketplace.make_offer.min_price","minprice",this.var_62.offerMinPrice.toString());
         this._localization.registerParameter("inventory.marketplace.make_offer.max_price","maxprice",this.var_62.offerMaxPrice.toString());
         if(param1 is FloorItem)
         {
            _loc4_ = this._roomEngine.getFurnitureImage(param1.type,new Vector3d(180,0,0),32,this,String(param1.extra),4293848814);
         }
         else
         {
            _loc4_ = this._roomEngine.getWallItemImage(param1.type,new Vector3d(180,0,0),32,this,param1.stuffData,4293848814);
         }
         if(!_loc4_)
         {
            return;
         }
         this.var_2584 = _loc4_.id;
         ...
         this.var_2583 = this._localization.getKey(_loc5_);
      ...
      {
         this._localization.registerParameter("inventory.marketplace.confirm_offer.info","furniname",this.var_2583);
         this._localization.registerParameter("inventory.marketplace.confirm_offer.info","price",this.var_1829.toString());
      ...
      private function confirmationCallback(param1:IConfirmDialog, param2:WindowEvent) : void
      {
         ...
         {
            this.var_62.makeOffer(this.var_1829);
         }
         this.var_62.releaseItem();
      }
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "buy_tokens_button":
                  this.var_62.buyMarketplaceTokens();
                  this.disposeView();
                  break;
               ...
               case "make_offer_button":
                  ...
                  if(_loc3_ != null)
                  {
                     this.var_1829 = parseInt(_loc3_.text);
      ...
      {
         var _loc1_:* = null;
         if(false && "ClubGiftController" == this._config.getKey("client.credits.embed.enabled"))
         {
      ...
      private function checkPrice() : void
      {
         ...
         if(_loc2_ > this.var_62.offerMaxPrice)
         {
            _loc1_.text = this.var_62.offerMaxPrice.toString();
            _loc2_ = this.var_62.offerMaxPrice;
         }
         var _loc3_:int = Math.max(Math.ceil(this.var_62.commission * 0.01 * _loc2_),1);
         ...
         if(_loc2_ < this.var_62.offerMinPrice)
         {
      ...
      {
         if(!this._view || !this._localization || !this.var_62)
         {
            return;
         }
         ...
         var _loc4_:int = Math.floor(param1 / (1 + this.var_62.commission * 0.01));
