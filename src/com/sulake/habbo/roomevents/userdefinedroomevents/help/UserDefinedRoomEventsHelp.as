package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_963:IContainerButtonWindow;
      
      private var var_964:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_85.getXmlWindow("ude_help"));
         this.var_1462 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_964 = this.var_1462.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_964)
         {
            this.var_964.procedure = this.onButtonClick;
         }
         this.var_963 = this.var_1462.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_963)
         {
            this.var_963.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_223 >= this._localizations.length - 1)
         {
            this.var_963.disable();
         }
         else
         {
            this.var_963.enable();
      ...
      {
         ++this.var_223;
         this.var_223 = Math.min(this.var_223,this._localizations.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_223;
         this.var_223 = Math.max(this.var_223,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this._localizations = [];
         var _loc1_:int = 1;
         while(this.var_85.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_85.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_85.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_85.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
            this._localizations.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this._localizations[this.var_223][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this._localizations[this.var_223][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this._localizations[this.var_223][1]).value;
      ...
      {
         if(!param1 || !this.var_85 || !this.var_85.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_85.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_85.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_85.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
