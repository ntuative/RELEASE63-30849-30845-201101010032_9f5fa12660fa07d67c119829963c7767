package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1128:Dictionary;
      ...
      private var var_725:ColorTransform;
      
      private var var_1394:ColorTransform;
      
      private var var_1794:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_715:Number = 1.0;
      
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         super();
         this.var_1128 = new Dictionary();
         this._visualization = param1;
         this.var_725 = new ColorTransform();
         this.var_725.alphaMultiplier = 1;
         this.var_1394 = new ColorTransform();
         this.var_1794 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_91 = this.var_1128[param1];
         this.var_919 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1793 >= 0)
         {
            this.var_252 = this._visualization.getSprite(this.var_1793);
            if(this.var_252 && this.var_252.asset)
            {
               ...
               if(this.var_83 == null)
               {
                  this.var_83 = this.var_252.asset.clone();
                  if(this.var_725.alphaMultiplier != 1)
                  {
                     this.var_602 = new BitmapData(this.var_83.width,this.var_83.height,true,4278190080);
                  }
               }
               this.var_2342 = -this.var_252.offsetX;
               this.var_2343 = -this.var_252.offsetY;
               this.var_252.asset = this.var_83;
            }
            if(this.var_83)
            {
            ...
            if(this.var_602)
            {
               this.var_602.fillRect(this.var_602.rect,4278190080);
            }
         }
      }
      ...
      {
         if(this.var_91 && this.var_91.roomObjectSpriteId == param1)
         {
            return this.var_91.y * this.var_1129;
         }
         return 0;
      }
      ...
      {
         if(!this.var_91 || !this.var_252)
         {
            return;
         }
         if(this.var_83 && this.var_252.asset != this.var_83)
         {
            this.var_252.asset = this.var_83;
         }
         if(this.var_919)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_919 && this.var_91.hasIgnited)
         {
            this.var_919 = true;
         }
         _loc8_ = this.var_1000 * this.var_1129;
         this.var_91.update();
         if(this.var_919)
         {
            if(this.var_91.roomObjectSpriteId >= 0)
            ...
            {
               this.var_83.draw(this.var_602,this.var_1794,this.var_725,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_91.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2342 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1129;
               _loc4_ = this.var_2343 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1129;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1394.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_83.draw(_loc7_,this._translationMatrix,this.var_1394,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         this.var_1793 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1000 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1129 = this.var_2534 / 64;
         this.var_715 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_715 = Math.min(this.var_715,1);
         this.var_725.alphaMultiplier = this.var_715;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "ClubGiftController" ? true : false;
               _loc21_ = [];
