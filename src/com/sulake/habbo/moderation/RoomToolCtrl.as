package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1033:IDropMenuWindow;
      
      private var var_430:ITextFieldWindow;
      
      private var var_610:Boolean = true;
      
      private var var_850:ICheckBoxWindow;
      
      private var var_1299:ICheckBoxWindow;
      
      private var var_1300:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_47.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_306));
         Logger.log("BEGINNING TO SHOW: " + this.var_306);
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_306);
         if(param1.flatId != this.var_306)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_306);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      {
         this.var_53 = IItemListWindow(this._frame.findChildByName("list_cont"));
         ...
         this.var_1033 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1033);
         this.var_1033.procedure = this.onSelectTemplate;
         this.var_850 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1299 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1300 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      {
         var _loc3_:IWindowContainer = IWindowContainer(this.var_53.getListItemByName(param2));
         ...
         if(!param1.exists)
         {
            this.disposeItemFromList(this.var_53,_loc3_);
            this.disposeItemFromList(this.var_53,this.var_53.getListItemByName("event_spacing"));
            return;
         }
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_47.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_47,WindowTracker.const_762,this._data.flatId),this._frame,false,false,true);
      }
      ...
      private function act(param1:Boolean) : void
      {
         ...
         var _loc2_:int = this.determineAction(param1,this.var_850.isSelected);
         this.var_47.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1501,_loc2_,this.var_430.text,"",""));
         if(this.var_1299.isSelected || this.var_1300.isSelected || this.var_850.isSelected)
         {
            this.var_47.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1299.isSelected,this.var_1300.isSelected,this.var_850.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1469);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1510);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_47.initMsg.roomMessageTemplates[this.var_1033.selection];
         if(_loc3_ != null)
         {
            this.var_610 = false;
            this.var_430.text = _loc3_;
         }
      }
   }
}
