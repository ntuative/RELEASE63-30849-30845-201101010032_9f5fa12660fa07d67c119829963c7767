package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         var _loc4_:* = null;
         if(this.var_211.indexOf(param1) >= 0)
         {
            return;
         }
         var _loc2_:String = param1.name.toLowerCase();
         var _loc3_:int = 0;
         while(_loc3_ < this.var_211.length)
         {
            _loc4_ = this.var_211[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               this.var_211.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      public function getFriendCount(param1:Boolean, param2:Boolean = false) : int
      {
         var _loc4_:* = null;
         var _loc3_:int = 0;
         for each(_loc4_ in this.var_211)
         {
      ...
      public function removeFriend(param1:int) : Friend
      {
         ...
         if(_loc2_ != null)
         {
            Util.remove(this.var_211,_loc2_);
            return _loc2_;
         }
         return null;
      }
      ...
      {
         if(this.var_903 >= this.getPageCount())
         {
            this.var_903 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_211.length / const_321);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_903 * const_321;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_903 + 1) * const_321,this.var_211.length);
      }
