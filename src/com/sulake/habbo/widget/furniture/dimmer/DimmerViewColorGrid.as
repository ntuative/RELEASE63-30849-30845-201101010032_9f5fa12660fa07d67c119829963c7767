package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_423:IItemGridWindow;
      
      private var _view:DimmerView;
      
      private var var_1454:XML;
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_423.numGridItems)
         {
            return;
         }
         this.select(this.var_423.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_423 == null)
         {
            return;
         }
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         {
            _loc2_ = this.var_339.getChildByName("chosen");
            if(_loc2_ != null)
            {
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_423.destroyGridItems();
         this.var_339 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_1454) as IWindowContainer;
            ...
            _loc3_.width = this.var_525.width;
            _loc3_.height = this.var_525.height;
            this.var_423.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_525.width,this.var_525.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_525,this.var_525.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_959.width,this.var_959.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_781.width,this.var_781.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_781,this.var_781.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_423.getGridItemIndex(param1.target as IWindow);
