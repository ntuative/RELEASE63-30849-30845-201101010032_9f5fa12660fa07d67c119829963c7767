package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_503:String = "InitConnection";
       
      
      private var var_149:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_126:SSOTicket;
      ...
      {
         this.var_149.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_149 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_149.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_134,HabboWindowStyle.const_37,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_149.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1575,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_149.habboConfiguration)
            {
               if(this.var_149.habboConfiguration.getKey("use.sso","false") == "ClubGiftController")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_149.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_149.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1575,"/");
         ...
         {
            dispatchEvent(new Event(const_503));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_149.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_126.removeEventListener(SSOTicket.const_327,this.onParsedTicketSuccess);
            this.var_126.removeEventListener(SSOTicket.const_289,this.onParsedTicketFailure);
            this.var_126 = null;
         }
         var _loc2_:String = "";
         if(this.var_149.habboConfiguration != null)
         {
            _loc2_ = this.var_149.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_149.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_126 = new SSOTicket(this.var_149.assets,_loc3_,_loc2_,param1);
         this.var_126.addEventListener(SSOTicket.const_327,this.onParsedTicketSuccess);
         this.var_126.addEventListener(SSOTicket.const_289,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_126.ticket);
         Logger.log("Has Facebook: " + this.var_126.isFacebookUser);
         if(this.var_126.isFacebookUser)
         {
            this.var_149.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_149.shockwaveClientUrl = this.var_126.shockwaveClientUrl;
         this.var_149.ssoTicket = this.var_126.ticket;
         this.var_126.removeEventListener(SSOTicket.const_327,this.onParsedTicketSuccess);
         this.var_126.removeEventListener(SSOTicket.const_289,this.onParsedTicketFailure);
         this.var_126 = null;
         dispatchEvent(new Event(const_503));
