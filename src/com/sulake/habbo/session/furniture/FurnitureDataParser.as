package com.sulake.habbo.session.furniture
{
   ...
   public class FurnitureDataParser extends EventDispatcher
   {
      ...
      public static const const_108:String = "ready";
      ...
      private var var_734:AssetLibrary;
      ...
      public function FurnitureDataParser(param1:Map, param2:Map, param3:Map, param4:ICoreLocalizationManager)
      {
         ...
         this.var_734 = new AssetLibrary("FurniDataParserAssetLib");
      }
      ...
      {
         var _loc2_:AssetLoaderStruct = this.var_734.loadAssetFromFile(param1,new URLRequest(param1),"text/plain");
         _loc2_.addEventListener(AssetLoaderEvent.const_30,this.parseFurnitureData);
      }
      ...
      private function parseFurnitureData(param1:AssetLoaderEvent) : void
      {
         ...
         for each(_loc10_ in _loc8_)
         {
            ...
            for each(_loc12_ in _loc11_)
            {
               ...
               {
                  _loc32_ = _loc24_[_loc25_];
                  if(_loc32_.charAt(0) == "#")
                  {
                     _loc32_ = _loc32_.replace("#","");
                     _loc23_.push(parseInt(_loc32_,16));
                  }
                  else
                  {
                     _loc23_.push(-parseInt(_loc32_));
                  }
                  _loc25_++;
               }
               _loc26_ = _loc13_[8];
               ...
               if(_loc14_ == "s")
               {
                  this.var_505.add(_loc15_,_loc31_);
               }
               else if(_loc14_ == "i")
               {
                  this._wallItems.add(_loc15_,_loc31_);
               }
               this.var_1864.add(_loc17_,_loc15_);
            }
            _loc9_++;
         }
         dispatchEvent(new Event(const_108));
      }
      ...
      public function registerFurnitureLocalizations() : void
      {
         ...
         for each(_loc1_ in this.var_505.getValues())
         {
