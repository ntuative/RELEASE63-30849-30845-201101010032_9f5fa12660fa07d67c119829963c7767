package com.sulake.habbo.session.handler
{
   ...
   public class RoomSessionHandler extends BaseHandler
   {
      ...
      public static const SESSION_CONNECTED:String = "RS_CONNECTED";
      
      public static const SESSION_READY:String = "RS_READY";
      
      public static const SESSION_DISCONNECTED:String = "RS_DISCONNECTED";
      ...
      private function onRoomConnected(param1:IMessageEvent) : void
      {
         ...
         var _loc3_:int = _xxxRoomCategory;
         if(listener)
         {
            listener.sessionUpdate(_loc2_,_loc3_,RoomSessionHandler.SESSION_CONNECTED);
         }
      }
      ...
      private function onFlatAccessible(param1:FlatAccessibleMessageEvent) : void
      {
         ...
         else if(listener && false)
         {
            ...
            if(_loc6_ != null)
            {
               listener.events.dispatchEvent(new RoomSessionDoorbellEvent(RoomSessionDoorbellEvent.const_40,_loc6_,_loc5_));
            }
         }
      }
      ...
      private function onRoomReady(param1:IMessageEvent) : void
      {
         ...
         if(listener)
         {
            ...
            listener.sessionUpdate(_loc2_,_loc3_,RoomSessionHandler.SESSION_READY);
         }
      }
      ...
      private function onFlatAccessDenied(param1:FlatAccessDeniedMessageEvent) : void
      {
         ...
         if(_loc5_ == null || _loc5_.length == 0)
         {
            if(listener)
            {
               listener.sessionUpdate(_loc2_,_loc3_,RoomSessionHandler.SESSION_DISCONNECTED);
            }
         }
         else if(listener && false)
         {
            ...
            if(_loc6_ != null)
            {
               listener.events.dispatchEvent(new RoomSessionDoorbellEvent(RoomSessionDoorbellEvent.const_138,_loc6_,_loc5_));
            }
         }
      }
      ...
      private function onRoomQueueStatus(param1:RoomQueueStatusMessageEvent) : void
      {
         ...
         for each(_loc9_ in _loc6_)
         {
            ...
            {
               _loc5_.method_4(_loc10_,_loc4_.getQueueSize(_loc10_));
            }
