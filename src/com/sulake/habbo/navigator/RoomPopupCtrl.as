package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_112:GuestRoomData;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2114 = new RoomThumbnailRenderer(param1);
         this.var_1065 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_171 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_171.visible = true;
         Util.hideChildren(this.var_171);
         this.refreshRoomName(this.var_171,this.var_112);
         if(this.var_112.ownerName != "" && this.var_112.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_171,!!this.var_112.event ? this.getEventCtg(this.var_112.categoryId) : this.getRoomCtg(this.var_112.categoryId));
         this.refreshRoomDesc(this.var_171,this.var_112);
         this.refreshExtraCont();
         this.refreshInfo(this.var_171,"trading_allowed",this.var_112.allowTrading);
         this.refreshInfo(this.var_171,"doormode_doorbell",this.var_112.doorMode == 1);
         this.refreshInfo(this.var_171,"doormode_password",this.var_112.doorMode == 2);
         Util.moveChildrenToColumn(this.var_171,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_171.height = Util.getLowestPoint(this.var_171);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_171,this.var_112.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_171.findChildByName("roomowner"));
         if(this.var_112.ownerName != "" && this.var_112.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_171.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1065.refreshTags(_loc1_,this.var_112.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_112.score);
         if(this.var_112.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_112.eventCreationTime);
         }
         this.var_2114.refreshThumbnail(_loc1_,this.var_112.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_249,Tabs.const_202,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
