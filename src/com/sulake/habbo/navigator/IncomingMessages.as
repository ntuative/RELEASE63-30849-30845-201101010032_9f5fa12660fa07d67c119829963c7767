   ...
   public class IncomingMessages
   {
      ...
      private function onUserRights(param1:IMessageEvent) : void
      {
         ...
         for each(_loc3_ in _loc2_)
         {
            ...
            if(_loc3_ == "select_staff_picked_rooms_in_client")
            {
               Logger.log("User can pick rooms for navigator");
      ...
      private function onOfficialRooms(param1:IMessageEvent) : void
      {
         ...
         this.data.officialRooms = _loc2_;
      ...
      private function onRoomEnter(param1:IMessageEvent) : void
      {
         ...
         {
            this._navigator.toolbar.events.dispatchEvent(new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_103,HabboToolbarIconEnum.ROOMINFO));
            this.setToolbarExitIcon();
         }
         if(!_loc2_.guestRoom)
         {
      ...
      private function onRoomInfo(param1:IMessageEvent) : void
      {
         ...
         {
            if(_loc2_.data.doorMode == RoomSettingsData.const_185 && this._navigator.sessionData.userName != _loc2_.data.ownerName)
            {
               this._navigator.doorbell.show(_loc2_.data);
            }
            else if(_loc2_.data.doorMode == RoomSettingsData.const_106 && this._navigator.sessionData.userName != _loc2_.data.ownerName)
            {
      ...
      {
         if(this._navigator.configuration.getKey("roomenterad.habblet.enabled") == "ClubGiftController" && false)
         {
      ...
      {
         var _loc1_:HabboToolbarSetIconEvent = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_96,HabboToolbarIconEnum.EXITROOM);
         var _loc2_:int = this.data == null ? 0 : int(this.data.homeRoomId);
         var _loc3_:int = this.data.enteredGuestRoom == null ? 0 : int(this.data.enteredGuestRoom.flatId);
         if(this.data.enteredGuestRoom != null || this.data.enteredPublicSpace != null)
         {
         ...
         else if(this.data != null && this.data.homeRoomId > 0)
         {
            _loc1_.iconState = "homeroom";
         }
         else
         {
            _loc1_ = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_156,HabboToolbarIconEnum.EXITROOM);
         }
         this._navigator.toolbar.events.dispatchEvent(_loc1_);
      }
      ...
      private function onFlatCreated(param1:IMessageEvent) : void
      {
         ...
         this._navigator.mainViewCtrl.reloadRoomList(Tabs.const_221);
      }
      ...
      private function onUserFlatCats(param1:IMessageEvent) : void
      {
         ...
         var _loc3_:RoomsTabPageDecorator = RoomsTabPageDecorator(this._navigator.tabs.getTab(Tabs.const_254).tabPageDecorator);
         _loc3_.prepareRoomCategories();
      }
      
      private function onRoomSettingsData(param1:IMessageEvent) : void
      {
         var parser:RoomSettingsDataMessageParser = null;
         var ctrl:RoomSettingsCtrl = null;
      ...
      private function onCanCreateRoom(param1:IMessageEvent) : void
      {
         ...
         else
         {
            ...
            if(this._navigator.sessionData.hasUserRight("fuse_can_create_more_rooms"))
            {
