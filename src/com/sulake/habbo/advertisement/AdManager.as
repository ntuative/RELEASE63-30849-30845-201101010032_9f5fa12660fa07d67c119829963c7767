package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_215:Timer;
      
      private var var_1428:String;
      
      private var var_1873:String;
      
      private var var_1169:BitmapData = null;
      
      private var var_1170:BitmapData = null;
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2658 = this._config.getKey("interstitial.enabled","false") == "ClubGiftController";
         this.var_2656 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2657 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1872 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_215 != null && this.var_215.running)
         {
            this.var_215.reset();
            return false;
         }
         if(this.var_1171 == 0)
         {
            this.var_1171 = getTimer();
            return false;
         }
         if(this.var_1872 > 0 && this.var_1871 >= this.var_1872)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1171;
         if(_loc1_ > this.var_2656)
         {
            ++this.var_1871;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1429 = _loc2_.imageUrl;
         this.var_2659 = _loc2_.clickUrl;
         this.var_1171 = getTimer();
         if(this.var_1429 != "")
         {
            _loc3_ = this.var_1429;
            ...
            {
               this.var_215 = new Timer(this.var_2657,1);
               this.var_215.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_215.running)
            {
               this.var_215.reset();
            }
            this.var_215.start();
            _loc4_ = new URLRequest(this.var_1429);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_497,0,0,param1.bitmapData,this.var_2659));
            this.var_1171 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_481));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1428 = _loc2_.imageUrl;
         this.var_1873 = _loc2_.clickUrl;
         if(this.var_1428 != "" && this.var_1873 != "")
         {
            _loc3_ = this.var_1428;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1428);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_399,0,0,_loc4_,this.var_1873,this.var_1169,this.var_1170));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1169 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1170 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
